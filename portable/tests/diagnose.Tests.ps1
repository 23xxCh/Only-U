...
Describe 'Only-U offline diagnose' {
    ...
    It 'instructs a TUI 运维会话 to show diagnosis and clean preview before confirmation' {
        ...
        $skill.IndexOf('portable\diagnose.ps1') | Should -BeGreaterThan $skill.IndexOf($sessionStartMarker)
        $skill.IndexOf('portable\clean.ps1') | Should -BeGreaterThan $skill.IndexOf('portable\diagnose.ps1')
        $skill.IndexOf($confirmationMarker) | Should -BeGreaterThan $skill.IndexOf('portable\clean.ps1')
