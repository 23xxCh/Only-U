...
# -Drive 必填：自动选第一个可移动盘可能选中 Only-U U 盘自身（2026-08-23 #39 评审），不再自动选。
param([Parameter(Mandatory = $false)][string]$Drive = '', [int]$SizeMB = 100)
...
if ($Drive -eq '') {
  ...
  $list = ($removable -join ' ')
  Write-Output "未指定盘符。请用 -Drive 明确指定要检测的 U 盘（当前可移动磁盘：$list）。"
  Write-Output '注意：不要对 Only-U 启动盘本身执行写满检测。'
  exit 1
}
